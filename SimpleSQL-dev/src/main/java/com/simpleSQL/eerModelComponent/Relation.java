...
/**
 * Represents a Relation component in the EER model with custom rendering. This
 * component appears as a diamond shape with rounded corners, a gradient
 * background, and a border that darkens when selected.
 */
public class Relation extends ComponentBase {
	...
	/**
	 * Paints the component with a custom gradient background and rounded diamond
	 * shape. Adjusts colors if the component is selected.
	 *
	...
	protected void paintComponent(Graphics g) {
		...
		GradientPaint gradient = new GradientPaint(0, 0, BACKGROUND_COLOR.brighter(), getWidth(), getHeight(),
				backgroundTemp);
		g2d.setPaint(gradient);

		// Define a diamond shape and apply rounded corners
		Polygon diamond = new Polygon();
		diamond.addPoint(getWidth() / 2, PADDING);
		diamond.addPoint(getWidth() - PADDING, getHeight() / 2);
		diamond.addPoint(getWidth() / 2, getHeight() - PADDING);
		diamond.addPoint(PADDING, getHeight() / 2);
	...
	/**
	 * Creates a rounded polygon shape with arcs at each corner, based on the
	...
	/**
	 * Creates a copy of this Relation component with the same properties, location,
	 * and size.
	 *
